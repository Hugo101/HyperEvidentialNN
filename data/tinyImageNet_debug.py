...
import random
# from HyperEvidentialNN.helper_functions import CustomDataset, AddLabelDataset
import csv
import math  
import time
...
class tinyImageNetVague():
    def __init__(
        ...
        self.train_ds_original = datasets.ImageFolder(train_dir)
        self.test_ds_original = datasets.ImageFolder(val_img_dir)
        ...
        self.train_split, self.valid_split = train_valid_split(self.train_ds_original, valid_perc=1-ratio_train, seed=seed)
        self.train_split_n = AddLabelDataset(self.train_split) #add an aditional label
        self.valid_split_n = AddLabelDataset(self.valid_split)
        self.test_ds_n = AddLabelDataset(self.test_ds_original)
        ...
        self.train_ds0 = CustomDataset(train_ds, transform=pre_norm_train)
        self.valid_ds = CustomDataset(valid_ds, transform=pre_norm_test)
        self.test_ds = CustomDataset(test_ds, transform=pre_norm_test)

        if self.duplicate:
            self.train_ds = self.modify_vague_samples(self.train_ds0)
        
        print(f'Data splitted. Train, Valid, Test size: {len(self.train_ds), len(self.valid_ds), len(self.test_ds)}')
        self.train_loader = DataLoader(self.train_ds, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=True)
        self.valid_loader = DataLoader(self.valid_ds, batch_size=batch_size, num_workers=num_workers, pin_memory=True)
        self.test_loader = DataLoader(self.test_ds, batch_size=batch_size, num_workers=num_workers, pin_memory=True)
