...
@torch.no_grad()
def evaluate_vague_nonvague(model, val_loader, kappa):
    model.eval()
    correct_vague = 0.0
    correct_nonvague = 0.0
    vague_total = 0
    ...
    for batch in val_loader:
        ...
        output = model(images)                  
        results = calculate_metrics(output, labels) #todo 
        correct_nonvague += results[0]
        correct_vague += results[1]
        nonvague_total += results[2]
        vague_total += results[3]
    return [correct_vague, correct_nonvague, vague_total, nonvague_total] #todo check this with calculate_metric
