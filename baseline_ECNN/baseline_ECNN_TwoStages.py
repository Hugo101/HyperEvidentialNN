...
def make(args, device):
    ...
    return mydata, model_DS, criterion
...
def train_ds_model(
    ...
    if optimr == "Nadam":
        optimizer = optim.NAdam(filter(lambda p : p.requires_grad, model.parameters()), lr=args.init_lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, momentum_decay=0.004)
    if optimr == "RMSprop": 
        optimizer = optim.RMSprop(filter(lambda p : p.requires_grad, model.parameters()), lr=args.init_lr)
    if optimr == "SGD": 
        optimizer = optim.SGD(filter(lambda p : p.requires_grad, model.parameters()), lr=args.init_lr)

    scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[700, 800], gamma=0.1)
    ...
    for epoch in range(num_epoch):
        ...
        if epoch % INTERVEL_EPOCH == 0 and epoch >= EPOCH_STAGE1:
            evaluate_vague_nonvague_final_ECNN(
                        model, mydata.test_loader, mydata.R, num_classes, device, 
                        nu=0.9, bestModel=False, epoch=epoch)
        time.sleep(0.5)
...
def evaluate_vague_nonvague_final_ECNN(
    ...
    test_result_log_ECNN(
        ...
        nonvague_acc, 
        bestModel=bestModel, epoch=epoch)
...
def generateSpecPath(
    ...
    tag0 = "_".join([f"{num_comp}M", f"ker{gauss_kernel_size}", "sweep_E-CNN_TwoStage"])
...
def main(project_name, args_all):
    ...
    with wandb.init(project=project_name, config=args_all):
        ...
        device = set_device(args.gpu)
        mydata, model, criterion = make(args, device)
        num_singles = mydata.num_classes
...
if __name__ == "__main__":
    # tell wandb to get started
    project_name = f"{opt['dataset']}-{opt['backbone']}-ECNN"
